...
# input options
my $usage = qq(
Usage:      batchFilter_N_AT_solexaPE.pl Parameters
...
Example:    batchFilter_N_AT_solexaPE.pl -solexaReadsPE_NO1 s_1_1_sequence.txt -solexaReadsPE_NO2 s_1_2_sequence.txt -adapterSequence adapterSequence.txt -blastresult1 blastresult1 -blastresult2 blastresult2 -filenamePrefix s_1 -readLength 36 -thresholdofConsecutiveN 5 -o /home/gene/escience/tmp/
\n);
...
die($usage) if ($solexaReads1 eq "" || $solexaReads2 eq "" || $adapterSeq eq "" || $blastresult1 eq "" || $blastresult2 eq "" || $prefix eq "" || $thresholdOfConsecutiveN eq "" || $o eq "");
...
my ($polyN_PE1, $polyN_PE2, $locPE1, $locPE2);
