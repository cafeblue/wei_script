...
Parameters: p               type 454 or solexa to specify platform type. (string)
            ...
            n               Threshold for the number of consecutive N in a read. Default: 5. (number)
...
Example:    batchQC.pl -p solexa -t T -sPE1 s_1_1_sequence.txt -sPE2 s_1_2_sequence.txt -ad adapterSequence.txt -n 5
\n);
...
if ($platform eq 454){
	...
	if ($readsType eq 'F'){
		...
		$bsubReturn = `bsub -q $q -n $c -o $jobLog -e $jobErr megablast -a $c -d /home/gene/bioinfo/data/bio_databases/ncbi/nt -i $fnaTmpFile -p 0.9 -e $evalue -o $megablastResult`;
		if ($bsubReturn =~ /^Job\s<(\d+)>\sis\ssubmitted/){
			&bsubDog($1);
		}else{
			open BSUBERR, ">$dataDirectoryErrlog" or die "$!";
...
# solexa reads
	...
	if ($readsType eq 'T'){
		...
		my $PE_trashConsecutiveN = $tmpRandDirectory.'PE_trashConsecutiveN';
		...
		#--------batchFilter_N_AT_solexaPE--------
		print "4: filter low-quality reads\n";
		my $statisticalReport = $tmpRandDirectory."PE"."_Statistical_Report.txt";
		$bsubReturn = `bsub -n 1 -q serial -o $jobLog -e $jobErr batchFilter_N_AT_solexaPE.pl -solexaReadsPE_NO1 $fastqTmpFile1 -solexaReadsPE_NO2 $fastqTmpFile2 -adapterSequence $adapterSeq -blastresult1 $blastn1 -blastresult2 $blastn2 -filenamePrefix PE -thresholdOfConsecutiveN $thresholdOfConsecutiveN -o $tmpRandDirectory`;
		if ($bsubReturn =~ /^Job\s<(\d+)>\sis\ssubmitted/){
			&bsubDog($1);
		}else{
			open BSUBERR, ">$dataDirectoryErrlog" or die "$!";
		...
		print "5: megablast\n";
		my $megablastResult = $tmpRandDirectory.'megablastResult';
		$bsubReturn = `bsub -q $q -n $c -o $jobLog -e $jobErr megablast -a $c -d /home/gene/bioinfo/data/bio_databases/ncbi/nt -i $PE_preserved_fasta -p 0.9 -e $evalue -o $megablastResult`;
		if ($bsubReturn =~ /^Job\s<(\d+)>\sis\ssubmitted/){
			&bsubDog($1);
		}else{
			open BSUBERR, ">$dataDirectoryErrlog" or die "$!";
		...
		my $nonPE_trashConsecutiveN = $tmpRandDirectory.'nonPE_trashConsecutiveN';
		...
		#--------batchFilter_N_AT_solexa--------
		print "4: filter low-quality reads\n";
		my $statisticalReport = $tmpRandDirectory."nonPE"."_Statistical_Report.txt";
		$bsubReturn = `bsub -n 1 -q serial -o $jobLog -e $jobErr batchFilter_N_AT_solexaNonPE.pl -solexaReads $fastqTmpFile -adapterSequence $adapterSeq -blastresult $blastn -filenamePrefix nonPE -thresholdOfConsecutiveN $thresholdOfConsecutiveN -o $tmpRandDirectory`;
		if ($bsubReturn =~ /^Job\s<(\d+)>\sis\ssubmitted/){
			&bsubDog($1);
		}else{
			open BSUBERR, ">$dataDirectoryErrlog" or die "$!";
		...
		print "5: megablast\n";
		my $megablastResult = $tmpRandDirectory.'megablastResult';
		$bsubReturn = `bsub -q $q -n $c -o $jobLog -e $jobErr megablast -a $c -d /home/gene/bioinfo/data/bio_databases/ncbi/nt -i $nonPE_single_fasta -p 0.9 -e $evalue -o $megablastResult`;
		if ($bsubReturn =~ /^Job\s<(\d+)>\sis\ssubmitted/){
			&bsubDog($1);
		}else{
			open BSUBERR, ">$dataDirectoryErrlog" or die "$!";
