...
open (RRPKM, ">/dev/shm/rpkm.matrix");
open (BP, ">/dev/shm/bp.matrix");
open (CC, ">/dev/shm/cc.matrix");
open (MF, ">/dev/shm/mf.matrix");
...
#        print $percent,'%', "\cM";
    ...
    close(CC);
    close(MF);
    system("Rscript ~/workdir/my_script/go.R");
    ...
    close(RRPKM);
    system("Rscript ~/workdir/my_script/rpkm.R");
...
#        }
    ...
    close(VENN);
    system("Rscript ~/workdir/my_script/venn.R");
