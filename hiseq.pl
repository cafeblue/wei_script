...
foreach (@reads_stat) {
	...
	if ($id =~ m/Undetermined_L00(\d)/) {
		...
		if ($id =~ m/_(\w+)_L00(\d)_R1_001\.fastq\.gz$/) {
			$known{$2} += $tmp1[1];
			$stat{$id} = {'reads' => $tmp1[1], 'bases' => $tmp1[2], 'ratio' => $tmp1[3], 'lane' => $2 , 'index' => $1, 'Q30' => $tmp1[4], 'Q20' => $tmp1[5], 'pf_cluster' => $tmp1[6], 'raw_cluster' => $tmp1[7], 'gc' => $tmp1[8]};
		}
		else {
...
#my $workbook  = Excel::Writer::XLSX->new( $infile );
