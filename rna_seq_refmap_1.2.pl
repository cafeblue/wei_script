...
# if the type of lib are different, DIE!!
if ($#insertsize > -1 && $#insertsize != $#seq_files) {
    die "\n\tDon\'t do that\! mix libraries are not support\!\n\n";
}
...
#                print $old_id{$lines[0]},"\t",$merged_cov{$old_id{$lines[0]}},"\n";
    ...
    if ($pnt eq "yes") {
        ...
        system("blastall -p blastn -d /share/data/database/NT/nt -e 1e-20 -a 16 -b 10 -m 9 -i new_transcript.fa -o new2nt.blastn");
        system("blastall -p blastx -d /share/data/database/Uniprot/uniprot_sprot.fasta -e 1e-20 -a 16 -b 10 -m 9 -i new_transcript.fa -o new2sw.blastx");
    ...
    foreach my $pair_diff (keys %pair_id_list) {
        ...
        print "\n$diff_sample1\t$diff_sample2\t$pair_diff\n";
        ...
        foreach my $row ( 1 .. $sheet -> {MaxRow}) {
         ...
         foreach my $acc_id (keys %transcriptome) {
             if ($transcriptome{$acc_id}{$pair_diff} <= 0.01 && $transcriptome{$acc_id}{'gene_name'} ne '-') {
